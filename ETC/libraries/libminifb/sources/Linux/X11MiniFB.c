...
mfb_open_ex(const char *title, int width, int height, int flags) {
	int depth = 0, i, formatCount = 0, convDepth = -1;
...
#endif
struct Window *
mfb_open_ex_buffer(void **buffer, const char *title, int width, int height, int flags) {
	int depth = 0, i, formatCount = 0, convDepth = -1;
	XPixmapFormatValues* formats;
	XSetWindowAttributes windowAttributes;
	XSizeHints sizeHints;
...
UpdateState mfb_update_buffer(struct Window *window)
{
	...
	if(window_data_x11->image->data == 0x0) {
		return STATE_INVALID_BUFFER;
	}
	...
	if(window_data_x11->image_scaler != 0x0) {
		stretch_image((uint32_t *)window_data_x11->image->data, 0, 0, window_data->buffer_width, window_data->buffer_height, window_data->buffer_width, (uint32_t *) window_data_x11->image_buffer, 0, 0, window_data->dst_width, window_data->dst_height, window_data->dst_width);
		window_data_x11->image_scaler->data = (char *) window_data_x11->image_buffer;
		XPutImage(window_data_x11->display, window_data_x11->window, window_data_x11->gc, window_data_x11->image_scaler, 0, 0, window_data->dst_offset_x, window_data->dst_offset_y, window_data->dst_width, window_data->dst_height);
	}
	else {
