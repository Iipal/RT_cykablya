		...
		register const size_t depth)
{
		if (depth < 32UL)
			return (attenuation * color(
				ray(point(record),
					normal(record) + random_in_unit_sphere()),
				hitables, depth + 1UL));
		else
			return (attenuation);
}
		...
		register const size_t depth)
{
	...
	if (condition(record) && material(record) & EMITER)
		return (albedo(*(hitables[iter(record)].sphere.material)));
	...
	else if (condition(record) && (material(record) & DIELECTRIC))
	{
		if (depth < 32UL)
		{
			...
			{
				refracted = refract_cond(normalize(direction(r)), -normal(record), ri(*(hitables[iter(record)].sphere.material)));
				if (refracted >= 0.0f)
					scattered = (random_float() < schlick(ri(*(hitables[iter(record)].sphere.material)) * dot(direction(r), normal(record)) / length(direction(r)), ri(*(hitables[iter(record)].sphere.material))))
					? ray(point(record), reflect(normalize(direction(r)), normal(record)))
					: ray(point(record), refract_do(normalize(direction(r)), -normal(record), ri(*(hitables[iter(record)].sphere.material)), refracted));
				else
			...
			{
				refracted = refract_cond(normalize(direction(r)), normal(record), (1.0f / ri(*(hitables[iter(record)].sphere.material))));
				if (refracted >= 0.0f)
					scattered = (random_float() < schlick(-dot(direction(r), normal(record)) / length(direction(r)), ri(*(hitables[iter(record)].sphere.material))))
					? ray(point(record), reflect(normalize(direction(r)), normal(record)))
					: ray(point(record), refract_do(normalize(direction(r)), normal(record), (1.0f / ri(*(hitables[iter(record)].sphere.material))), refracted));
				else
					scattered = (random_float() < 1.0f)
					? ray(point(record), reflect(normalize(direction(r)), normal(record)))
					: ray(point(record), refract_do(normalize(direction(r)), normal(record), (1.0f / ri(*(hitables[iter(record)].sphere.material))), refracted));
	...
	else
	{
		...
		return (vec(1.0f, 1.0f, 1.0f) - factor + vec(0.38f, 1.078f, 3.0f) * factor);
		// return (vec(0.7f, 0.8f, 3.0f));
	}
}
