	...
	main(void)
{
	...
	static const size_t			screen_width = 2560UL / 2UL;
	static const size_t			screen_height = 1080UL / 4UL;
	...
	t_material_sf				materials[] = {
		...
		material(COLOR,			vec(0.6f, 0.7f, 0.1f)),
		material(NORMAL,		vec(0.0f, 0.0f, 0.0f)),
		material(EMITER,		vec(4.0f, 4.0f, 4.0f)),
	};
	...
	t_sphere_sf					spheres[] = {
		...
		sphere(vec(-2.0f, 0.0f, -1.0f),		0.5f),
		sphere(vec(2.0f, 0.0f, -1.0f),		0.5f),
		sphere(vec(0.0f, 25.25f, -1.0f),	10.25f)
	};
	...
	union u_hitables			hitables[] = {
		(union u_hitables){ { GENERIC, 9, NULL, NULL } },
		...
		(union u_hitables){ { CONE, 0, cones + 0UL, materials + 3UL } },
		(union u_hitables){ { CONE, 0, cones + 1UL, materials + 3UL } },
		(union u_hitables){ { CONE, 0, cones + 2UL, materials + 3UL } },
		// (union u_hitables){ { SPHERE, 0, spheres + 5UL, materials + 4UL } },
		// (union u_hitables){ { SPHERE, 0, spheres + 6UL, materials + 5UL } },
		(union u_hitables){ { SPHERE, 0, spheres + 7UL, materials + 6UL } },
	};
	...
	while (-42)
	{
		...
		// 		Params[i].screen_width = screen_width;
		// 		Params[i].screen_height = screen_height;
		// 		Params[i].start = (__typeof__(Params->start))(render_part * i);
		// 		Params[i].stop = (__typeof__(Params->stop))(render_part * (i + 1));
		// 		Params[i].step = (__typeof__(Params->step))(1UL);
		// 		Params[i].hitables = (__typeof__(Params->hitables))(hitables);
		...
		state = mfb_update(window, screen);
		// state = mfb_update_buffer(window);
		if (state == STATE_EXIT)
			// break ;
			_Exit(0) ;
	}
